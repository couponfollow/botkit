...
function Facebookbot(configuration) {
    ...
    facebook_botkit.defineBot(function(botkit, config) {
         ...
         */
        bot.findConversationByCustomId = function(custom_id) {
            botkit.debug('CUSTOM FIND CONVO', custom_id);

            for (var t = 0; t < botkit.tasks.length; t++) {
                for (var c = 0; c < botkit.tasks[t].convos.length; c++) {
                    if (
                        botkit.tasks[t].convos[c].isActive() &&
                        botkit.tasks[t].convos[c].custom_id == custom_id
                    ) {
                        botkit.debug('FOUND EXISTING CONVO!');
                    ...
                    for (var m = 0; m < obj.entry[e].messaging.length; m++) {
                        ...
                        if (facebook_message.message) {
                            ...
                            if (facebook_message.message.quick_reply) {
                                ...
                                message.text = facebook_message.message.quick_reply.payload;
                            }
                        ...
                        } else if (facebook_message.payment) {
                            var message = {
                                user: facebook_message.sender.id,
                                channel: facebook_message.sender.id,
                                timestamp: facebook_message.timestamp,
                                payment: facebook_message.payment
                            };
